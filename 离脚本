...
local bulletTrackingConnection = nil
...
if not root and LocalPlayer.Character then
    root = LocalPlayer.Character:WaitForChild("HumanoidRootPart", 5)
end
...
local function toggleFly(state)
    ...
    if isFlying then
        if not root or not root.Parent then
            if LocalPlayer.Character then
                root = LocalPlayer.Character:WaitForChild("HumanoidRootPart", 3)
            end
...
local function AddActiveStroke(stroke)
    if stroke and stroke.Parent then activeStrokes[stroke] = true end
end
...
local function CreateSidebarButton(text, pageName)
    ...
    button.MouseButton1Down:Connect(function() button.BackgroundColor3 = Color3.fromRGB(70, 70, 70) end)
    button.MouseButton1Up:Connect(function() button.BackgroundColor3 = Color3.fromRGB(60, 60, 60) end)
...
local function CreateFunctionButton(text, parent, toggleCallback)
    ...
    button.MouseEnter:Connect(function() if not isEnabled then button.BackgroundColor3 = Color3.fromRGB(60, 60, 60) end end)
    button.MouseLeave:Connect(UpdateButtonColors)
    button.MouseButton1Down:Connect(function() if not isEnabled then button.BackgroundColor3 = Color3.fromRGB(70, 70, 70) end end)
            ...
            if player ~= LocalPlayer then
                ...
                playerBtn.MouseButton1Click:Connect(function()
                    if selectionCallback then
                        ...
                        for _ in pairs(selectedList) do selectedCount = selectedCount + 1 end
...
LocalPlayer.CharacterAdded:Connect(function(newChar)
    ...
    isFlying = false
    if flyToggle then flyToggle.Text = "关" end
